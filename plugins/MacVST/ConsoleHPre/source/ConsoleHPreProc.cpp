...
void ConsoleHPre::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
	...
	double bezRez = pow(1.0-ATK, 8.0) / overallscale; 
	double sloRez = pow(1.0-RLS,12.0) / overallscale;
	sloRez = fmin(fmax(sloRez-(bezRez*0.5),0.00001),1.0);
	bezRez = fmin(fmax(bezRez,0.0001),1.0);
	double gate = pow(pow(GAT,4.0),sqrt(bezCThresh+1.0));
	//Dynamics2
	...
	while (--sampleFrames >= 0)
    {
		...
		if (fabs(inputSampleL) > gate+(sloRez*bezGateL)) bezGateL = ((bezGateL*overallscale*3.0)+3.0)*(0.25/overallscale);
		else bezGateL = fmax(0.0, bezGateL-(sloRez*sloRez));
		if (fabs(inputSampleR) > gate+(sloRez*bezGateR)) bezGateR = ((bezGateR*overallscale*3.0)+3.0)*(0.25/overallscale);
		else bezGateR = fmax(0.0, bezGateR-(sloRez*sloRez));
		bezCompF[bez_cycle] += bezRez;
		bezCompF[bez_SampL] += (fabs(inputSampleL) * bezRez);
		bezCompF[bez_SampR] += (fabs(inputSampleR) * bezRez);
		bezMaxFL = fmax(bezMaxFL,fabs(inputSampleL));
		bezMaxFR = fmax(bezMaxFR,fabs(inputSampleR));
		
		if (bezCompF[bez_cycle] > 1.0) {
			bezCompF[bez_cycle] -= 1.0;
			
			if (bezMaxFL < gate) bezCompF[bez_SampL] = bezMaxFL/gate; //note: SampL is a control voltage,
			...
			if (bezMaxFR < gate) bezCompF[bez_SampR] = bezMaxFR/gate; //note: SampR is a control voltage,
		...
		if (bezGateL < 1.0 && gate > 0.0) {inputSampleL *= bezGateL;}
		if (bezGateR < 1.0 && gate > 0.0) {inputSampleR *= bezGateR;}
