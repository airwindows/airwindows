...
AudioEffect* createEffectInstance(audioMasterCallback audioMaster) {return new kCyberCity(audioMaster);}

kCyberCity::kCyberCity(audioMasterCallback audioMaster) :
...
VstInt32 kCyberCity::getVendorVersion () {return 1000;}
void kCyberCity::setProgramName(char *name) {vst_strncpy (_programName, name, kVstMaxProgNameLen);}
void kCyberCity::getProgramName(char *name) {vst_strncpy (name, _programName, kVstMaxProgNameLen);}
...
VstInt32 kCyberCity::getChunk (void** data, bool isPreset)
{
...
VstInt32 kCyberCity::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{	
...
void kCyberCity::setParameter(VstInt32 index, float value) {
    switch (index) {
...
float kCyberCity::getParameter(VstInt32 index) {
    switch (index) {
...
void kCyberCity::getParameterName(VstInt32 index, char *text) {
    switch (index) {
...
void kCyberCity::getParameterDisplay(VstInt32 index, char *text) {
    switch (index) {
...
void kCyberCity::getParameterLabel(VstInt32 index, char *text) {
    switch (index) {
...
VstInt32 kCyberCity::canDo(char *text) 
{ return (_canDo.find(text) == _canDo.end()) ? -1: 1; } // 1 = yes, -1 = no, 0 = don't know

bool kCyberCity::getEffectName(char* name) {
    vst_strncpy(name, "kCyberCity", kVstMaxProductStrLen); return true;
}

VstPlugCategory kCyberCity::getPlugCategory() {return kPlugCategEffect;}

bool kCyberCity::getProductString(char* text) {
  	vst_strncpy (text, "airwindows kCyberCity", kVstMaxProductStrLen); return true;
}

bool kCyberCity::getVendorString(char* text) {
