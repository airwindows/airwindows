...
AudioEffect* createEffectInstance(audioMasterCallback audioMaster) {return new kPlateD(audioMaster);}

kPlateD::kPlateD(audioMasterCallback audioMaster) :
...
kPlateD::~kPlateD() {}
VstInt32 kPlateD::getVendorVersion () {return 1000;}
void kPlateD::setProgramName(char *name) {vst_strncpy (_programName, name, kVstMaxProgNameLen);}
void kPlateD::getProgramName(char *name) {vst_strncpy (name, _programName, kVstMaxProgNameLen);}
...
VstInt32 kPlateD::getChunk (void** data, bool isPreset)
{
...
VstInt32 kPlateD::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{	
...
void kPlateD::setParameter(VstInt32 index, float value) {
    switch (index) {
...
float kPlateD::getParameter(VstInt32 index) {
    switch (index) {
...
void kPlateD::getParameterName(VstInt32 index, char *text) {
    switch (index) {
...
void kPlateD::getParameterDisplay(VstInt32 index, char *text) {
    switch (index) {
...
void kPlateD::getParameterLabel(VstInt32 index, char *text) {
    switch (index) {
...
VstInt32 kPlateD::canDo(char *text) 
{ return (_canDo.find(text) == _canDo.end()) ? -1: 1; } // 1 = yes, -1 = no, 0 = don't know

bool kPlateD::getEffectName(char* name) {
    vst_strncpy(name, "kPlateD", kVstMaxProductStrLen); return true;
}

VstPlugCategory kPlateD::getPlugCategory() {return kPlugCategEffect;}

bool kPlateD::getProductString(char* text) {
  	vst_strncpy (text, "airwindows kPlateD", kVstMaxProductStrLen); return true;
}

bool kPlateD::getVendorString(char* text) {
