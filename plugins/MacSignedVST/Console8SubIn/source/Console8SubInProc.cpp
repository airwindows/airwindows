...
void Console8SubIn::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		if (softL[9] > 0.73824539) inputSampleL = softL[4]+(fabs(softL[4])*sin(softL[9]-0.73824539)*0.26175461);
		if (-softL[9] > 0.73824539) inputSampleL = softL[4]-(fabs(softL[4])*sin(-softL[9]-0.73824539)*0.26175461);
		...
		if (softR[9] > 0.73824539) inputSampleR = softR[4]+(fabs(softR[4])*sin(softR[9]-0.73824539)*0.26175461);
		if (-softR[9] > 0.73824539) inputSampleR = softR[4]-(fabs(softR[4])*sin(-softR[9]-0.73824539)*0.26175461);
		//Console8 slew soften: must be clipped or it can generate NAN out of the full system
...
void Console8SubIn::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
