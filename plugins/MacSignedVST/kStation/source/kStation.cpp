...
AudioEffect* createEffectInstance(audioMasterCallback audioMaster) {return new kStation(audioMaster);}

kStation::kStation(audioMasterCallback audioMaster) :
...
kStation::~kStation() {}
VstInt32 kStation::getVendorVersion () {return 1000;}
void kStation::setProgramName(char *name) {vst_strncpy (_programName, name, kVstMaxProgNameLen);}
void kStation::getProgramName(char *name) {vst_strncpy (name, _programName, kVstMaxProgNameLen);}
...
VstInt32 kStation::getChunk (void** data, bool isPreset)
{
...
VstInt32 kStation::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{	
...
void kStation::setParameter(VstInt32 index, float value) {
    switch (index) {
...
float kStation::getParameter(VstInt32 index) {
    switch (index) {
...
void kStation::getParameterName(VstInt32 index, char *text) {
    switch (index) {
...
void kStation::getParameterDisplay(VstInt32 index, char *text) {
    switch (index) {
...
void kStation::getParameterLabel(VstInt32 index, char *text) {
    switch (index) {
...
VstInt32 kStation::canDo(char *text) 
{ return (_canDo.find(text) == _canDo.end()) ? -1: 1; } // 1 = yes, -1 = no, 0 = don't know

bool kStation::getEffectName(char* name) {
    vst_strncpy(name, "kStation", kVstMaxProductStrLen); return true;
}

VstPlugCategory kStation::getPlugCategory() {return kPlugCategEffect;}

bool kStation::getProductString(char* text) {
  	vst_strncpy (text, "airwindows kStation", kVstMaxProductStrLen); return true;
}

bool kStation::getVendorString(char* text) {
