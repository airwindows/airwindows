...
MV2::MV2(audioMasterCallback audioMaster) :
...
MV2::~MV2() {}
VstInt32 MV2::getVendorVersion () {return 1000;}
void MV2::setProgramName(char *name) {vst_strncpy (_programName, name, kVstMaxProgNameLen);}
void MV2::getProgramName(char *name) {vst_strncpy (name, _programName, kVstMaxProgNameLen);}
...
VstInt32 MV2::getChunk (void** data, bool isPreset)
{
...
VstInt32 MV2::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{	
...
void MV2::setParameter(VstInt32 index, float value) {
    switch (index) {
...
float MV2::getParameter(VstInt32 index) {
    switch (index) {
...
void MV2::getParameterName(VstInt32 index, char *text) {
    switch (index) {
...
void MV2::getParameterDisplay(VstInt32 index, char *text) {
    switch (index) {
...
void MV2::getParameterLabel(VstInt32 index, char *text) {
    switch (index) {
...
VstInt32 MV2::canDo(char *text) 
{ return (_canDo.find(text) == _canDo.end()) ? -1: 1; } // 1 = yes, -1 = no, 0 = don't know

bool MV2::getEffectName(char* name) {
    vst_strncpy(name, "MV2", kVstMaxProductStrLen); return true;
}

VstPlugCategory MV2::getPlugCategory() {return kPlugCategEffect;}

bool MV2::getProductString(char* text) {
  	vst_strncpy (text, "airwindows MV2", kVstMaxProductStrLen); return true;
}

bool MV2::getVendorString(char* text) {
