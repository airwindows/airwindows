/* ========================================
 *  ConsoleX3Pre - ConsoleX3Pre.h
 *  Copyright (c) airwindows, Airwindows uses the MIT license
 * ======================================== */

#ifndef __ConsoleX3Pre_H
#include "ConsoleX3Pre.h"
#endif

void ConsoleX3Pre::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		if (!compBypass) {
			...
			if (compress) {
				inputSampleL *= (bezComp[bez_compL]/bezThresh);
				inputSampleR *= (bezComp[bez_compR]/bezThresh);
			} else {
				inputSampleL /= bezThresh;
				inputSampleR /= bezThresh;
			}
			double ctrlL = fmin(fabs(inputSampleL),sqrThresh*bezComp[bez_compL]*0.6180339887498949);
			double ctrlR = fmin(fabs(inputSampleR),sqrThresh*bezComp[bez_compR]*0.6180339887498949);
			bezComp[bez_minL] = fmax(bezComp[bez_minL]-sloRez,ctrlL);
			bezComp[bez_minR] = fmax(bezComp[bez_minR]-sloRez,ctrlR);
			bezComp[bez_CtrlL] += (bezComp[bez_minL] * bezRez);
			bezComp[bez_CtrlR] += (bezComp[bez_minR] * bezRez);
			bezComp[bez_cycle] += bezRez;
			if (bezComp[bez_cycle] > bezTrim) {bezComp[bez_cycle] = 0.0;
				bezComp[bez_CL] = bezComp[bez_BL]; bezComp[bez_BL] = bezComp[bez_AL];
				bezComp[bez_AL] = bezComp[bez_CtrlL]; bezComp[bez_CtrlL] = 0.0;
				bezComp[bez_CR] = bezComp[bez_BR]; bezComp[bez_BR] = bezComp[bez_AR];
				bezComp[bez_AR] = bezComp[bez_CtrlR]; bezComp[bez_CtrlR] = 0.0;}
			double X = bezComp[bez_cycle];
			bezComp[bez_compL] = bezComp[bez_BL]+(bezComp[bez_CL]*(1.0-X)*(1.0-X))+(bezComp[bez_BL]*2.0*(1.0-X)*X)+(bezComp[bez_AL]*X*X);
			bezComp[bez_compR] = bezComp[bez_BR]+(bezComp[bez_CR]*(1.0-X)*(1.0-X))+(bezComp[bez_BR]*2.0*(1.0-X)*X)+(bezComp[bez_AR]*X*X);
			bezComp[bez_compL] = ((1.0-(fmin(bezComp[bez_compL],0.9999))));
			bezComp[bez_compR] = ((1.0-(fmin(bezComp[bez_compR],0.9999))));
			if (compress) {
				inputSampleL = inputSampleL*(1.0-invDry)*bezComp[bez_compL]*bezThresh;
				...
				inputSampleL += dryCompL*invDry*(1.0-(bezComp[bez_compL]*(1.0-invDry)));
				inputSampleR = inputSampleR*(1.0-invDry)*bezComp[bez_compR]*bezThresh;
				...
				//this is a degenerate form of a Taylor Series to approximate sin()
				inputSampleR += dryCompR*invDry*(1.0-(bezComp[bez_compR]*(1.0-invDry)));
			} else {
				inputSampleL = ((inputSampleL/(0.1+bezThresh))*(1.0-invDry))/bezComp[bez_compL];
				inputSampleL = fmax(fmin(inputSampleL,2.305929007734908),-2.305929007734908);
				double addtwo = inputSampleL * inputSampleL;
				double empower = inputSampleL * addtwo; // inputSampleL to the third power
				inputSampleL -= (empower / 6.0); empower *= addtwo; // to the fifth power
				...
				//this is a degenerate form of a Taylor Series to approximate sin()				
				inputSampleL += dryCompL*invDry*(1.0-(bezComp[bez_compL]*(1.0-invDry)));
				inputSampleR = ((inputSampleR/(0.1+bezThresh))*(1.0-invDry))/bezComp[bez_compR];
				inputSampleR = fmax(fmin(inputSampleR,2.305929007734908),-2.305929007734908);
				addtwo = inputSampleR * inputSampleR;
				empower = inputSampleR * addtwo; // inputSampleR to the third power
				inputSampleR -= (empower / 6.0); empower *= addtwo; // to the fifth power
...
void ConsoleX3Pre::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
