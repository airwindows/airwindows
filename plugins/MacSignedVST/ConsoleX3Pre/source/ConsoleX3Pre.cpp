...
AudioEffect* createEffectInstance(audioMasterCallback audioMaster) {return new ConsoleX3Pre(audioMaster);}

ConsoleX3Pre::ConsoleX3Pre(audioMasterCallback audioMaster) :
...
VstInt32 ConsoleX3Pre::getVendorVersion () {return 1000;}
void ConsoleX3Pre::setProgramName(char *name) {vst_strncpy (_programName, name, kVstMaxProgNameLen);}
void ConsoleX3Pre::getProgramName(char *name) {vst_strncpy (name, _programName, kVstMaxProgNameLen);}
...
VstInt32 ConsoleX3Pre::getChunk (void** data, bool isPreset)
{
...
VstInt32 ConsoleX3Pre::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{	
...
void ConsoleX3Pre::setParameter(VstInt32 index, float value) {
    switch (index) {
...
float ConsoleX3Pre::getParameter(VstInt32 index) {
    switch (index) {
...
void ConsoleX3Pre::getParameterName(VstInt32 index, char *text) {
    switch (index) {
...
void ConsoleX3Pre::getParameterDisplay(VstInt32 index, char *text) {
    switch (index) {
...
void ConsoleX3Pre::getParameterLabel(VstInt32 index, char *text) {
    switch (index) {
...
VstInt32 ConsoleX3Pre::canDo(char *text) 
{ return (_canDo.find(text) == _canDo.end()) ? -1: 1; } // 1 = yes, -1 = no, 0 = don't know

bool ConsoleX3Pre::getEffectName(char* name) {
    vst_strncpy(name, "ConsoleX3Pre", kVstMaxProductStrLen); return true;
}

VstPlugCategory ConsoleX3Pre::getPlugCategory() {return kPlugCategEffect;}

bool ConsoleX3Pre::getProductString(char* text) {
  	vst_strncpy (text, "airwindows ConsoleX3Pre", kVstMaxProductStrLen); return true;
}

bool ConsoleX3Pre::getVendorString(char* text) {
